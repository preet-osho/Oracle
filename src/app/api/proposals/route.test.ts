...
describe('Proposals API /api/proposals', () => {
  beforeEach(() => { vi.clearAllMocks(); setupChain(); });

  it('returns proposals limited to 10', async () => {
    const c = setupChain({ data: [{ id: 'prop-1', brief: 'Test' }] });
    const res = castMockResponse(await GET());
    expect(from).toHaveBeenCalledWith('proposals');
    expect(c.limit).toHaveBeenCalledWith(10);
    expect(res.body).toEqual([{ id: 'prop-1', brief: 'Test' }]);
  });
  ...
  it('creates a proposal', async () => {
    const c = setupChain({ data: { id: 'prop-1', brief: 'Test proposal' } });
    const res = castMockResponse(await POST(createPostRequest({ brief: 'Test proposal', domain: 'seo' }) as any));
    expect(c.insert).toHaveBeenCalled();
    expect(res.body).toEqual({ id: 'prop-1', brief: 'Test proposal' });
  });
  it('returns 500 on insert error', async () => { setupChain({ data: null, error: new Error('Insert failed') }); const res = castMockResponse(await POST(createPostRequest({ brief: 'X' }) as any)); expect(res.init).toEqual({ status: 500 }); });
